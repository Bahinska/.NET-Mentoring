...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        {
            for (int size = 10; size <= 1000; size += 5)
            {
                ...
                watch.Stop();
                var regularElapsed = watch.ElapsedMilliseconds;

                watch.Restart();
                _ = matrixMultiplierParallel.Multiply(m1, m2);
                watch.Stop();
                var parallelElapsed = watch.ElapsedMilliseconds;

                Console.WriteLine($"Matrix size: {size}x{size}, Regular: {regularElapsed}ms, Parallel: {parallelElapsed}ms");

                if (parallelElapsed < regularElapsed)
                {
                    Console.WriteLine($"Parallel multiplication is faster from {size}x{size} size");
                    break;
                }
            }
        }
